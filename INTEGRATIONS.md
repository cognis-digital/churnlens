...
**churnlens** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `churnlens` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
churnlens ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
churnlens ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
churnlens ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `churnlens` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="churnlens") for rec in records]   # records = your JSON output
